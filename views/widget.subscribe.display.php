                ...
                if ( isset( $order_list ) ) {
                        ...
                        if ( ! empty( $customfields[ $field_key ] ) ) {
                            ...
                            if ( ! empty( $instance[ 'show_' . $name_stripped . '_name' ] ) ) {
                                ...
                                if ( 'select' === $customfields[ $field_key ]['sailthru_customfield_type'] ) {
                                    ...
                                    echo '<select ' . ( sailthru_field_class( $customfields[ $field_key ]['sailthru_customfield_class'] ) ) . // phpcs:ignore WordPress.Security.EscapeOutput
                                        ' ' . esc_attr( sailthru_attributes( $attributes ) ) . 'name="custom_' . esc_attr( $name_stripped ) . 
                                        '" id="sailthru_' . esc_attr( $name_stripped ) . '_id" class="form-control">';
                                    ...
                                    foreach ( $items as $item ) {
                                        if ( ! empty( $item ) ) {
                                            ...
                                            echo 'type="radio" name="custom_' . esc_attr( $name_stripped ) . '" value="' . esc_attr( $vals[1] ) . '" ' . 
                                                ( sailthru_field_class( $customfields[ $field_key ]['sailthru_customfield_class'], $customfields[ $field_key ]['sailthru_customfield_type'] ) ) . // phpcs:ignore WordPress.Security.EscapeOutput
                                                ' ' . esc_attr( sailthru_attributes( $attributes ) ) . '> ' . esc_html( $vals[0] ) . '';
                                        }
                                    }
                                    ...
                                    foreach ( $items as $item ) {
                                        ...
                                        if ( ! empty( $item ) ) {
                                            ...
                                            echo 'type="checkbox" name="custom_' . esc_attr( $name_stripped ) .  (count( $items) > 1 ? '[]' : '') . '" value="' . esc_attr( $vals[1] ) . '"  ' . 
                                                ( sailthru_field_class( $customfields[ $field_key ]['sailthru_customfield_class'], $customfields[ $field_key ]['sailthru_customfield_type'] ) ) . // phpcs:ignore WordPress.Security.EscapeOutput
                                                ' ' . esc_attr( sailthru_attributes( $attributes ) ) . '> ' . esc_html( $vals[0] ) . '';
                                        }
                                    }
                                    ...
                                    if ( 'checked' === $instance[ 'show_' . $name_stripped . '_required' ] ) {
                                        ...
                                        echo '<input ' . ( sailthru_field_class( $customfields[ $field_key ]['sailthru_customfield_class'] ) ) . // phpcs:ignore WordPress.Security.EscapeOutput
                                            ' type="' . esc_attr( $customfields[ $field_key ]['sailthru_customfield_type'] ) . '" ';
                                        ...
                                        echo ( sailthru_field_class( $customfields[ $field_key ]['sailthru_customfield_class'] ) ) . // phpcs:ignore WordPress.Security.EscapeOutput
                                            ' type="' . esc_attr( $customfields[ $field_key ]['sailthru_customfield_type'] ) . '" ' . esc_attr( sailthru_attributes( $attributes ) ) . 'name="custom_' . esc_attr( $name_stripped ) . '" id="sailthru_' . esc_attr( $name_stripped ) . '_id"  class="form-control"/>';
                    ...
                    for ( $i = 0; $i < $key; $i++ ) {
                        ...
                        if ( ! empty( $customfields[ $field_key ] ) ) {
                            ...
                            if ( 'select' === $customfields[ $field_key ]['sailthru_customfield_type'] ) {
                                echo '<label for="custom_' . esc_attr( $name_stripped ) . '">' . esc_html( $customfields[ $field_key ]['sailthru_customfield_label'] ) . '</label>
                                    <select ' . ( sailthru_field_class( $customfields[ $field_key ]['sailthru_customfield_class'] ) ) . // phpcs:ignore WordPress.Security.EscapeOutput
                                    ' ' . esc_attr( sailthru_attributes( $attributes ) ) . 'name="custom_' . esc_attr( $name_stripped ) . '" id="sailthru_' . esc_attr( $name_stripped ) . '_id" class="form-control">';
                                $items = explode( ',', $customfields[ $field_key ]['sailthru_customfield_value'] );
